...
## 1. 사용자 식별 방법 🔑
---
핵심: MySQL의 사용자 계정은 **사용자 아이디** 뿐만 아니라 **접속 지점(호스트명이나 도메인 또는 IP주소)**도 함께 확인한다.<br>
예시) `root@localhost`

![예시 사진.png](example.png)
...
---
계정의 HOST를 `%` 로 설정하자. (와일드카드 느낌이다.)

### 1-2. 이를 활용한 사례
---
**[상황]**<br>
한 대의 MySQL 서버에서 개발용 데이터베이스와 운영용 데이터베이스를 따로 구축해서 사용했다.
그리고 개발용 EC2 인스턴스와 운영용 EC2 인스턴스에서 각각 DB용 인스턴스로 요청을 보냈다.

![인프라 아키텍처의 일부.png](infra_architecture.png)

**[적용]**<br>
MySQL의 계정을 `celuveat_prod@{ec2_prod_ip}`, `celuveat_dev@{ec2_dev_ip}` 하나씩 생성했고<br>
이로 인해 의도하지 않은 상황 발생을 방지했다.

### 1-3. 계정 식별에서의 주의할 점
---
만약 `user` 이라는 계정이 두 개가 있다.<br>
한 계정의 HOST는 `192.168.1.01`로 지정되어 있고, 한 계정의 HOST는 `%`로 모두 허용했다고 가정해보자.

이때 `192.168.1.01`에서 `user` 계정으로 접속을 시도하면 어떻게 될까?<br>
...
> 💡 근데 이렇게 설정하는 경우는 가능한 자제하는 게 좋아보인다.<br>
> 만약 불가피한 경우에는 `--host` 속성으로 계정을 명시해서 접속 시도할 수 있을 것이다.

> 💡 아이디과 호스트가 같은 계정 2개를 만드는 일은 하지 말자.

이때 주의할 점은, 인증에 실패했다고 자동으로 더 넓은 범위의 계정으로 접속을 시도하지 않는다.
...
![실험 환경.png](test_environment.png)

그리고 kdkdhoho 계정으로 로그인을 시도했을 때, 성공하는 계정을 보자.

![로그인 결과.png](after_test.png)

HOST를 `localhost`로 설정한 계정으로 로그인이 성공한다.

### 1-5. 실험2: 실험1에 `%`이 추가된다면?
---
![실험2 환경.png](test2_environment.png)

만약 위와 같이 `kdkdhoho@%` 계정이 추가된다면 어떤 계정으로 접속하게 될까?<br>
(참고로 위 계정의 암호는 `xyz`로 설정했다.)

아마 예상하기 쉽겠지만 `localhost`로만 접속이 가능하다.

> 💡 HOST의 범위는 `localhost` < 지정 IP < `%` 인 것 같다.<br>
> 참고로 추가 실험 결과, `127.0.0.1`도 `localhost`에 순위가 밀린다.

## 2. 사용자 계정 관리
---
계정을 관리할 수 있는 계정과 그렇지 않은 계정,<br>
그리고 

### 2-1. 시스템 계정과 일반 계정
---
MySQL 8.0부터는 `SYSTEM_USER` 권한을 가지고 있냐에 따라 `시스템 계정`과 `일반 계정`으로 구분된다.

> 시스템 계정이란?
> - MySQL 서버에서 내부적으로 실행되는 백그라운드 스레드와는 무관하다.
> - 일반 계정과 같이 사용자, 그 중에서 *데이터베이스 서버 관리자* 를 위한 계정이다.
...
- `mysql.sys@localhost`: 8.0부터 기본으로 내장된 **sys 스키마의 객체들의 DEFINER**로 사용하는 계정
  > sys: Performance Schema를 편하게 볼 수 있도록 별도로 만든 뷰의 모음
...
계정 인증 방식과 비밀번호를 설정한다. => `IDENTIFIED WITH {계정 인증 방식} BY {비밀번호}`
...
하지만 `REQUIRE` 옵션을 `SSL`로 설정하지 않아도, `Caching SHA-2 Authentication` 인증 방식을 사용하면 암호화된 채널만으로 서버에 접속하게 된다.
...
근데 만약 계정의 비밀번호를 바꾼다면 서비스에 타격이 갈 것이다.<br>
결국 하나의 암호를 몇 년 동안 사용하게 될 것이다.
...
-- GRANT OPTION 권한은 마지막에 WITH GRANT OPTION을 추가한다.
